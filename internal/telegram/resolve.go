...
// ErrDialogScanLimit marks a dialog warm that hit its page cap
// (warmDialogsMaxPages per folder) before exhausting the folder. It never
// reaches a caller — a partial warm is treated as a cache miss like any
// other — but it keeps the completion stamp unlatched, so the next lookup
// rescans instead of trusting an incomplete scan.
var ErrDialogScanLimit = errors.New("dialog list is larger than the numeric-ID resolver scans")
