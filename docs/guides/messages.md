...
`tg_messages_delete` therefore takes `scheduled` (default `false`). With the flag, the IDs are read as queue entries and dropped from the queue; without it, they are read as history and the published messages are deleted. Passing a queued ID without the flag deletes whatever is published under that number: the call succeeds, reports the usual `Requested deletion of N message(s)`, and the queued message stays where it was. `tg_messages_edit` reaches the published history only — a queued message is changed by deleting it and sending a replacement with `scheduleDate`.

The two paths differ in what they can honestly report back. Telegram's `updateDeleteScheduledMessages` names the scheduled IDs it actually removed, so a `scheduled: true` call gets a `deleted` field: the server's own count, not the number of ids the call passed — an id already gone from the queue contributes nothing to it, so a two-id call that returns `deleted: 0` means neither existed where the call looked, no follow-up read needed to tell a real removal from a no-op. Ordinary history deletion has no equivalent signal: Telegram's response carries a `pts_count`, but that field belongs to the update-sequence mechanism, not to a count of what the call actually removed, and even Telegram's own reference client does not use it that way. A `scheduled: false` call therefore omits `deleted` entirely rather than presenting an unverified number as server truth; its `output` text says only that the deletion was requested. A caller that needs confirmation for a history delete has to read the messages back.
