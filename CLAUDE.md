...
User-facing documentation lives in `docs/`, is built with MkDocs Material (`mkdocs.yml`) and publishes to <https://mcp-tg.lexfrei.dev> on every push to master (`.github/workflows/pages.yml`). The same `mkdocs build --strict` runs on every PR as a **step of the `Test` job**, not as a job of its own, and `TestDocsGates_RunInARequiredJob` pins it — and the Markdown lint beside it — there. The reason is that `master`'s required status checks are `Lint`, `Test`, `Build (amd64)` and `Build (arm64)`: a standalone `Docs` job is not one of them and nothing in the graph depends on it, so a PR that broke the docs would merge green, `pages.yml` would fail on master, and the site would quietly stop updating with the last good build still served. Relative links inside `docs/` (`[Peers](peers.md)`, `building.md#transport-modes`) are checked by that build and by nothing else — `TestDocsSiteURLs_ResolveToPages` reads absolute URLs only — so the build has to block rather than inform. Keep it inside a required job; adding a new check to branch protection is a repository setting nobody is obliged to remember. The custom domain lives in the repository's Pages settings, NOT in a `CNAME` file: GitHub ignores `CNAME` entirely when the site publishes from a custom Actions workflow ("no `CNAME` file is created, and any existing `CNAME` file is ignored and is not required"). A committed `CNAME` would look like the mechanism while being inert, sending anyone whose domain drops to read the wrong file. It is the one piece of site configuration outside version control — along with `Enforce HTTPS`, which belongs on once the first deploy has issued the certificate. README.md is a landing page only — install, quickstart, links — and anything longer belongs on a docs page.

```bash
pip install --requirement requirements-docs.txt   # pinned; MkDocs 2.0 drops the plugin system with no migration path
mkdocs serve
npx --yes markdownlint-cli2@0.23.0 "docs/**/*.md"  # what the Documentation Validation check runs, as an action
```
...
- Any major-version promise, on any page (`TestDocsMajorVersion_MatchesTheModulePath`): a `vN>=2` tag needs a matching `/vN` suffix in go.mod. Reads every page, since the rationale lives in `docs/messages.md` now, not the README.
