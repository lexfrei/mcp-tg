...
`HOMEBREW_TAP_GITHUB_TOKEN` (a PAT with write access to the tap) must exist as a repository secret. A missing secret expands to an empty string, GoReleaser builds a GitHub client with an empty token, and the tap push fails — leaving the release with archives but no formula. The snapshot gate cannot catch this, because it never publishes. Note also that `create-release` publishes the release BEFORE the archives are uploaded, so any failure in the final job leaves a public release with nothing attached; the gate makes that unlikely rather than impossible.
