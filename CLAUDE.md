...
`messagesFromUpdates` (albums, forwards) shares `extractNewMessage` with the single-message path so scheduled sends (`updateNewScheduledMessage`) are not silently dropped — a scheduled album used to come back with `count: 0`. Only albums can be scheduled; `tg_messages_forward` has no `scheduleDate`. An echo that cannot be read at all (unhandled shape, no matching update) is repaired by `echoOrSubmitted` with the submitted entity set, so an unreadable echo never masquerades as "the server applied no formatting" and send the caller into an endless correction loop.
