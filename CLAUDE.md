...
The formula ships an env file at `etc/mcp-tg/mcp-tg.env` and a wrapper in `libexec` that sources it, and the service runs the WRAPPER. This is not decoration: a service manager injects only the variables the unit declares, credentials cannot live in a public formula, and `launchctl setenv` dies with the login session — so a service running the bare binary comes up unconfigured after every reboot and `keep_alive` restarts it forever. The formula's `test do` block asserts three things at once (terminates without touching the network, exit code 1, the `TELEGRAM_APP_ID is required` sentence); `TestBrewFormulaContract_NoCredentialsExitsOneWithoutNetwork` pins all three from this repo, because the tap is where a break would surface and nobody reads it.
