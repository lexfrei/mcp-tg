...
- Every docs dependency is pinned to an exact version, in `requirements-docs.txt` and nowhere else (`TestMkdocsMaterialPin_LivesInRequirements`). A version restated in CLAUDE.md or README is a second thing to bump, so the test rejects a direct `pip install mkdocs...` in either — an unpinned or disagreeing install renders a different site than CI publishes.
